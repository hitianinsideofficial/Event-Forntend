...
import { fetchEventById, submitRegistrationApi } from '../../../services/api';
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white flex flex-col">
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {/* Conditional View: Registration Form OR Generated Ticket */}
        {ticket ? (
          ...
          <div className="glass-panel p-8 max-w-xl mx-auto text-center border-2 border-emerald-500/30 shadow-2xl shadow-emerald-500/10 animate-fadeIn">
            ...
            <p className="text-xs text-slate-400 mb-6">Here is your official QR Event Pass for entry.</p>
            ...
            <div className="bg-slate-900/80 p-4 rounded-xl text-left text-xs space-y-2 mb-6 border border-white/5">
              <div className="flex justify-between"><span className="text-slate-400">Attendee Name:</span> <span className="font-semibold text-white">{ticket.fullName}</span></div>
              <div className="flex justify-between"><span className="text-slate-400">Email:</span> <span className="font-semibold text-white">{ticket.email}</span></div>
              <div className="flex justify-between"><span className="text-slate-400">Event:</span> <span className="font-semibold text-indigo-300">{ticket.eventTitle}</span></div>
              <div className="flex justify-between"><span className="text-slate-400">Status:</span> <span className="px-2 py-0.5 rounded bg-amber-500/20 text-amber-300 font-medium">Pending Check-in</span></div>
              {ticket.files && ticket.files.length > 0 && (
                <div className="flex justify-between items-center pt-2 border-t border-white/5">
                  <span className="text-slate-400">Uploaded Media:</span>
          ...
          <div className="glass-panel p-6 sm:p-8 max-w-2xl mx-auto border border-white/10">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/10">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">Please provide your details below.</p>
              </div>
              {event.hasAttendance && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* File Upload (PNGs, Videos, Docs) */}
              <div className="form-group">
                <label className="form-label">
                  Upload Submission File (PNG, JPG, MP4 Video, PDF, ZIP)
                </label>
                <div className="relative border-2 border-dashed border-white/20 rounded-xl p-4 text-center hover:border-indigo-500/50 transition-colors bg-slate-900/40">
                  <input 
                    type="file" 
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[10px] text-slate-500">Max file size 50MB. Uploads stored automatically in Google Drive.</p>
                  </div>
                </div>
              </div>
              ...
              <button 
                type="submit" 
                disabled={submitting}
                className="btn-primary w-full py-3 justify-center text-sm font-bold shadow-indigo-500/30 mt-6"
              >
